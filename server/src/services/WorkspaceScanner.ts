...
export const WORKSPACE_ROOT = resolve(
  process.env.TTY_PROJECTS_ROOT ?? join(process.env.HOME ?? '/home/dev', 'projects')
)

export interface Workspace {
  // Path relative to WORKSPACE_ROOT. Nested workspaces carry a '/', e.g.
  // "linkflow/proposals-api".
  name: string
  path: string
...
// added after an earlier version happily reported `tradingrush/__pycache__` as
...
// — projects/linkflow is exactly this, holding five projects. Scanning only one
...
export function scanWorkspaces(force = false): Workspace[] {
  ...
  for (const name of top) {
    ...
    // "where do I want a shell", and `projects/linkflow` is a real answer: a
